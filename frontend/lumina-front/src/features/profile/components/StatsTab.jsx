...
import { 
  ...
  Target, Layers, Award, Clock, Lock 
} from 'lucide-react';
...
const StatsTab = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* График активности - показываем разное для своего и чужого профиля */}
      <div>
        ...
        <div className={`p-6 rounded-xl ${themeClasses.colors.bg.secondary}`}>
          {isOwnProfile ? (
            activityData && activityData.length > 0 && activityData.some(day => day.count > 0) ? (
              <>
                    ...
                    <div key={idx} className="flex flex-col items-center w-1/7">
                      <div className="relative w-full flex justify-center mb-2 group">
                        <div 
                          ...
                          style={{ 
                            height: `${Math.max(4, (day.count / Math.max(...activityData.map(d => d.count), 1)) * 80)}px`,
